...
public class ContactMeCreateService {
    private static final Logger logger = Logger.getLogger(ContactMeCreateService.class.getName());
    ...
    public ContactMe createContactMe(ContactMeRequest contactMeRequest) {
        ...
        var count = contactMeCountByEmailAndStatusPendingRepository.countContactMeByEmailAndStatusPending(
                contactMeRequest.email());
        if (count > 0) {
